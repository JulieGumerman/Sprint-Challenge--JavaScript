...
const tyrannosaurus = {
  name: "tyrannosaurus, but known to friends as simply 'Fluffy'",
  diet: "carnivorous",
  weight: "7000kg",
  length: "12m",
...
*/
const animalNames = zooAnimals.map((animal) => {
  return `Name: ${animal.animal_name}, Species: ${animal.scientific_name}`
});
console.table(animalNames);
console.log("The .forEach() method does not work like this, so I used .map() instead")
...
const nameArray = zooAnimals.map((animal) => {
  return animal.animal_name;
});

const lowerCase = nameArray.map((name) => {
